...
::destRecord(struct in_addr const& addr, Port const& port, u_int8_t ttl,
	     destRecord* next, bool igmpReq, bool master)
  : fNext(next), fGroupEId(addr, port.num(), ttl), fPort(port), fRefCnt(0), fIgmpRefCnt(0), fMaster(master) {
  if (igmpReq) { fIgmpRefCnt = 1; } else { fRefCnt = 1; }
}
...
void Groupsock::addDestination(struct in_addr const& addr, Port const& port, bool igmpReq /* = false */, bool ownerSession /* = true */) {
  ...
  for (destRecord* dests = fDests; dests != NULL; dests = dests->fNext) {
	...
	&& port.num() == dests->fPort.num()) {
      if (igmpReq) { dests->fIgmpRefCnt++; } else { dests->fRefCnt = 1; }

      DBG_S1(printf("GSOCK-ADD [%x:%d]: REF: %d IGMPREF:%d [igmp:%d] [owner:%d]\n", addr.s_addr, ntohs(port.num()), dests->fRefCnt, dests->fIgmpRefCnt, igmpReq, ownerSession);)
      return;
    }
  }

  DBG_S1(printf("GSOCK-ADD [%x:%d] NEW (1) igmp:%d owner:%d\n", addr.s_addr, ntohs(port.num()), igmpReq, ownerSession);)
  fDests = new destRecord(addr, port, ttl(), fDests, igmpReq, ownerSession);
}

void Groupsock::removeDestination(struct in_addr const& addr, Port const& port, bool igmpReq /* = false */, bool ownerSession /* = true */) {
  for (destRecord** destsPtr = &fDests; *destsPtr != NULL;
	...
	&& port.num() == (*destsPtr)->fPort.num()) {
      ...
      if (igmpReq) {
        if ((*destsPtr)->fIgmpRefCnt > 0) {
          (*destsPtr)->fIgmpRefCnt--;
        }
      } else if ((*destsPtr)->fRefCnt > 0 && (!(*destsPtr)->fMaster || ownerSession)) {
        (*destsPtr)->fRefCnt--;
      }

      DBG_S1(printf("GSOCK-REM [%x:%d]: REF: %d IGMPREF: %d [igmp:%d] [owner:%d]\n", addr.s_addr, ntohs(port.num()), (*destsPtr)->fRefCnt, (*destsPtr)->fIgmpRefCnt, igmpReq, ownerSession);)

      if ((igmpReq && (*destsPtr)->fRefCnt == 0 && (*destsPtr)->fIgmpRefCnt == 0) ||
          (!igmpReq && (*destsPtr)->fRefCnt == 0)) {
        // Remove the record pointed to by *destsPtr :
        destRecord* next = (*destsPtr)->fNext;
        (*destsPtr)->fNext = NULL;
        delete (*destsPtr);
        *destsPtr = next;
        DBG_S1(printf("-->DELETE GSOCK\n");)
      }
      return;
    }
  }
  DBG_S1(printf("GSOCK-REM [%x:%d] -NOT-FOUND- igmp:%d owner:%d\n", addr.s_addr, ntohs(port.num()), igmpReq, ownerSession);)
}
...
void Groupsock::setTTL(u_int8_t newTTL)
{
  if (newTTL < 255) {
    fTTL = newTTL;
  }
