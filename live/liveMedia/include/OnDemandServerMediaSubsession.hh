...
protected:
  ...
  static unsigned int rtcpAppPacketHandler(void* clientData, unsigned char const** appPacketBuffer);
  virtual unsigned int rtcpAppPacketHandler1(unsigned char const** appPacketBuffer);
...
public:
  Destinations(struct in_addr const& destAddr,
    ...
    : isTCP(False), addr(destAddr), rtcpAddr(destAddr), rtpPort(rtpDestPort), rtcpPort(rtcpDestPort) {
  }
  Destinations(struct in_addr const& destAddr,
               struct in_addr const& rtcpPeerAddr,
               Port const& rtpDestPort,
               Port const& rtcpDestPort)
    : isTCP(False), addr(destAddr), rtcpAddr(rtcpPeerAddr), rtpPort(rtpDestPort), rtcpPort(rtcpDestPort) {
  }
...
public:
  ...
  Boolean areCurrentlyPlaying() { return fAreCurrentlyPlaying; }
