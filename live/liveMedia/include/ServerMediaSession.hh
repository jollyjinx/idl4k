...
public:
  ...
  virtual char* generateSDPSessionLevel(); // Note: The caller is responsible for freeing the returned string
  virtual const char* generateSDPMediaLevel();
  ...
  void setOwnerSessionId(unsigned sessionId) { assert(fOwnerSessionId==0); if (fOwnerSessionId==0) fOwnerSessionId = sessionId; }
  unsigned getOwnerSessionId() { return fOwnerSessionId; }

  Boolean fStreamingToMulticast;
  in_addr_t fMulticastAddress;
