...
static isMyIpAddress_f _isMyIpAddress = NULL;

void setIsMyIpAddressInterface(isMyIpAddress_f isMyAddress)
{
  _isMyIpAddress = isMyAddress;
}
...
Boolean parseRTSPRequestString(char const* reqStr,
  ...
  for (; (int)j < (int)(reqStrSize-8); ++j) {
	...
	&& reqStr[j+4] == ':' && reqStr[j+5] == '/') {
      ...
      if (reqStr[j] == '/') {
        ...
        ++j;
        char reqIpAddress[24] = {0};
        char reqIpPort[8] = {0};
        unsigned int reqIpAddressDone = 0;
        unsigned int reqIpAddressPos = 0;
        unsigned int reqIpPortPos = 0;
        if (_isMyIpAddress) {
          while (j < reqStrSize && reqStr[j] != '/' && reqStr[j] != ' ') {
            if (reqStr[j] != ':') {
              if (!reqIpAddressDone && reqIpAddressPos < sizeof(reqIpAddress)-1) {
                reqIpAddress[reqIpAddressPos] = reqStr[j];
                reqIpAddressPos++;
              } else if (reqIpAddressDone && reqIpPortPos < sizeof(reqIpPort)-1) {
                reqIpPort[reqIpPortPos] = reqStr[j];
                reqIpPortPos++;
              }
            } else {
              reqIpAddressDone = 1;
          ...
          reqIpAddress[reqIpAddressPos] = 0;
          reqIpPort[reqIpPortPos] = 0;
          if (strlen(reqIpAddress) > 0) {
            if (!_isMyIpAddress(reqIpAddress, reqIpPort))
            {
              return False;
            }
          } else {
  ...
  for (j = i; (int)j < (int)(reqStrSize-5); ++j) {
    if ((reqStr[j] == 'C' || reqStr[j] == 'c') && (reqStr[j+1] == 'S' || reqStr[j+1] == 's') && (reqStr[j+2] == 'E' || reqStr[j+2] == 'e') &&
	(reqStr[j+3] == 'Q' || reqStr[j+3] == 'q') && reqStr[j+4] == ':') {
      j += 5;
      while (j < reqStrSize && (reqStr[j] ==  ' ' || reqStr[j] == '\t')) ++j;
      unsigned n;
  ...
  for (j = i; (int)j < (int)(reqStrSize-15); ++j) {
    if ((reqStr[j] == 'C' || reqStr[j] == 'c') && (reqStr[j+1] == 'O' || reqStr[j+1] == 'o') && (reqStr[j+2] == 'N' || reqStr[j+2] == 'n') && (reqStr[j+3] == 'T' || reqStr[j+3] == 't') && (reqStr[j+4] == 'E' || reqStr[j+4] == 'e') &&
	(reqStr[j+5] == 'N' || reqStr[j+5] == 'n') && (reqStr[j+6] == 'T' || reqStr[j+6] == 't') && reqStr[j+7] == '-' && (reqStr[j+8] == 'L' || reqStr[j+8] == 'l') &&
	(reqStr[j+9] == 'E' || reqStr[j+9] == 'e') && (reqStr[j+10] == 'N' || reqStr[j+10] == 'n') && (reqStr[j+11] == 'G' || reqStr[j+11] == 'g') && (reqStr[j+12] == 'T' || reqStr[j+12] == 't') && (reqStr[j+13] == 'H' || reqStr[j+13] == 'h') &&
	reqStr[j+14] == ':') {
      j += 15;
      while (j < reqStrSize && (reqStr[j] ==  ' ' || reqStr[j] == '\t')) ++j;
      unsigned num;
