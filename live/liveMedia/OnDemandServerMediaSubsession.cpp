...
::OnDemandServerMediaSubsession(UsageEnvironment& env,
  ...
  : ServerMediaSubsession(env),
    fSDPLines(NULL), fLastStreamToken(NULL), fInitialPortNum(initialPortNum), fReuseFirstSource(reuseFirstSource) {
...
char const*
OnDemandServerMediaSubsession::sdpLines(bool setControl /* = false */) {
  if (fSDPLines == NULL) {
...
void OnDemandServerMediaSubsession::startStream(unsigned clientSessionId,
  ...
  if (streamState != NULL) {
    streamState->startPlaying(destinations,
			      ...
			      serverRequestAlternativeByteHandler, serverRequestAlternativeByteHandlerClientData, igmpReq, (fParentSession->getOwnerSessionId() == clientSessionId));
...
void OnDemandServerMediaSubsession::pauseStream(unsigned clientSessionId,
						void* streamToken, bool igmpReq /* = false */) {

  Destinations* destinations = (Destinations*)(fDestinationsHashTable->Lookup((char const*)clientSessionId));
  StreamState* streamState = (StreamState*)streamToken;

  if (streamState != NULL && destinations != NULL)
  {
    streamState->endPlaying(destinations, igmpReq, (fParentSession->getOwnerSessionId() == clientSessionId));
  }
}
...
void OnDemandServerMediaSubsession::deleteStream(unsigned clientSessionId,
  ...
  if (destinations != NULL) {
    ...
    if (streamState != NULL) streamState->endPlaying(destinations, false, (fParentSession->getOwnerSessionId() == clientSessionId));
  }

  if (fParentSession->getOwnerSessionId() == clientSessionId)
  {
    while (1) {
      Destinations* dst = (Destinations*)(fDestinationsHashTable->RemoveNext());
      if (dst == NULL) break;
      if (streamState != NULL) streamState->endPlaying(dst, false, (fParentSession->getOwnerSessionId() == clientSessionId));
      delete dst;
    }
    if (streamState) {
      streamState->reclaim();
...
unsigned int OnDemandServerMediaSubsession::rtcpAppPacketHandler(void* clientData, unsigned char const** appPacketBuffer)
{
  OnDemandServerMediaSubsession* sms = (OnDemandServerMediaSubsession*)clientData;
  return sms->rtcpAppPacketHandler1(appPacketBuffer);
}

unsigned int OnDemandServerMediaSubsession::rtcpAppPacketHandler1(unsigned char const** appPacketBuffer)
{
  return 0;
}
...
::startPlaying(Destinations* dests,
  ...
  if (dests->isTCP) {
    ...
    if (fRTPgs != NULL) fRTPgs->addDestination(dests->addr, dests->rtpPort, igmpReq, ownerSession);
    if (fRTCPgs != NULL) fRTCPgs->addDestination(dests->addr, dests->rtcpPort, igmpReq, ownerSession);
    if (fRTCPInstance != NULL) {
      fRTCPInstance->setSpecificRRHandler(dests->rtcpAddr.s_addr, dests->rtcpPort,
					  rtcpRRHandler, rtcpRRHandlerClientData);
      fRTCPInstance->setAppPacketHandler(fMaster.rtcpAppPacketHandler, &fMaster);
    }
  }
...
void StreamState::endPlaying(Destinations* dests, bool igmpReq /* = false */, bool ownerSession /* = true */) {
  if (dests->isTCP) {
    ...
    if (fRTPgs != NULL) fRTPgs->removeDestination(dests->addr, dests->rtpPort, igmpReq, ownerSession);
    if (fRTCPgs != NULL) fRTCPgs->removeDestination(dests->addr, dests->rtcpPort, igmpReq, ownerSession);
    if (fRTCPInstance != NULL) {
...
void StreamState::reclaim() {
  ...
  if (fRTCPInstance) {
    fRTCPInstance->setAppPacketHandler(NULL, NULL);
