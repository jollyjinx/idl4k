...
ServerMediaSession::ServerMediaSession(UsageEnvironment& env,
  ...
  : Medium(env), fStreamingToMulticast(False), fMulticastAddress(0), fIsSSM(isSSM), fSubsessionsHead(NULL),
    fSubsessionsTail(NULL), fSubsessionCounter(0),
    fReferenceCount(0), fDeleteWhenUnreferenced(False), fOwnerSessionId(0) {
...
char* ServerMediaSession::generateSDPSessionLevel() {
  return NULL;
}

const char* ServerMediaSession::generateSDPMediaLevel() {
  return NULL;
}
