...
void MPEG2TransportStreamFramer::afterGettingFrame1(unsigned frameSize,
...
#if 0 /* sat2ip should send empty rtp packets so the frameSize=0 is a valid value */
  if (fFrameSize == 0) {
  ...
  if (fFrameSize && syncBytePosition == fFrameSize) {
...
::updateTSPacketDurationEstimate(unsigned char* pkt, double timeNow) {
  ...
  ++fTSPacketCount;
#if 1 /* hot-fix: it is useless when reading data from demux so set always the packet duration to 0 */
  fTSPacketDurationEstimate = 0;
  return;
#else
