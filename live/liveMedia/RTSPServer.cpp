...
void RTSPServer::RTSPClientSession::handleRequestBytes(int newBytesRead) {
  ...
  if (!fSessionIsActive) {
    ...
    {
      //delete this;
      envir().taskScheduler().rescheduleDelayedTask(fLivenessCheckTask, 0, (TaskFunc*)livenessTimeoutTask, this);
    }
...
::handleCmd_DESCRIBE(char const* cseq,
  ...
  do {
    ...
    /* sometimes the vlc can call describe and then crash/leave;
       for the sat>ip compatibility mode this line below ensures that the session will be close when timeout occurs
     */
...
static int parseTransportHeader(char const* buf,
				 ...
				 Boolean& multicastReq) {
  // Initialize the result parameters to default values:
  static const unsigned STREAMING_MODE_VALID=(1<<0);
  static const unsigned ALL_FIELDS_VALID=(STREAMING_MODE_VALID);
  ...
  while (sscanf(fields, "%[^\n\r;]", field) == 1) {
    if (strcmp(field, "RTP/AVP/TCP") == 0) {
      ...
      validFields|=STREAMING_MODE_VALID;
    ...
    } else if (multicastReq && sscanf(field, "port=%hu-%hu", &p1, &p2) == 2) {
	clientRTPPortNum = p1;
	clientRTCPPortNum = streamingMode == RAW_UDP ? 0 : p2; // ignore the second port number if the client asked for raw UDP
  ...
  delete[] field;
  return (validFields == ALL_FIELDS_VALID)?0:-2;
}
...
::handleCmd_SETUP(char const* cseq,
  ...
  do {
    ...
    if (fStreamStates == NULL) {
      ...
      if (fOurServerMediaSession->getOwnerSessionId() != fOurSessionId) {
        fOurServerMediaSession->incrementReferenceCount();
      }
    ...
    int parseTransportHeaderResult = parseTransportHeader(fullRequestStr, streamingMode, streamingModeString,
                                                          clientsDestinationAddressStr, clientsDestinationTTL,
                                                          clientRTPPortNum, clientRTCPPortNum,
                                                          rtpChannelId, rtcpChannelId, fIsMulticast);
    if (parseTransportHeaderResult)
    {
      delete[] streamingModeString;
      if (parseTransportHeaderResult == -1) {
        handleCmd_bad(cseq);
      } else {
    ...
    if (serverRTPPort.num() == 0) {
      delete[] streamingModeString;
      snprintf((char*)fResponseBuffer, sizeof fResponseBuffer, "HTTP/1.0 455 Method Not Valid in This State\r\n%s\r\n\r\n", dateHeader());
      break;
    }
