...
CROSS_PLATFORM_NAME=axe-linux
CROSS_ROOTFS_PREFIX=/opt/STM/STLinux-2.4/devkit/sh4/target
...
# cross-compile target specific stuff
case $CROSS_PLATFORM_NAME in
	axe-linux)
		OS_NAME="AXELinux"
		OS_VERSION="1.0"
		TIVO="/*#define TIVO_SUPPORT*/"
...
[ ! -e "$CROSS_ROOTFS_PREFIX/usr/include/sqlite3.h" ] && MISSING="libsqlite3 $MISSING"
[ ! -e "$CROSS_ROOTFS_PREFIX/usr/include/jpeglib.h" ] && MISSING="libjpeg $MISSING"
[ ! -e "$CROSS_ROOTFS_PREFIX/usr/include/libexif/exif-loader.h" ] && MISSING="libexif $MISSING"
[ ! -e "$CROSS_ROOTFS_PREFIX/usr/include/id3tag.h" ] && MISSING="libid3tag $MISSING"
[ ! -e "$CROSS_ROOTFS_PREFIX/usr/include/ogg/ogg.h" ] && MISSING="libogg $MISSING"
[ ! -e "$CROSS_ROOTFS_PREFIX/usr/include/vorbis/codec.h" ] && MISSING="libvorbis $MISSING"
[ ! -e "$CROSS_ROOTFS_PREFIX/usr/local/include/FLAC/metadata.h" ] && MISSING="libflac $MISSING"
[ ! -e "$CROSS_ROOTFS_PREFIX/usr/local/include/ffmpeg/avutil.h" -a \
  ! -e "$CROSS_ROOTFS_PREFIX/usr/local/include/libavutil/avutil.h" -a \
  ! -e "$CROSS_ROOTFS_PREFIX/usr/local/include/ffmpeg/libavutil/avutil.h" ] && MISSING="libavutil $MISSING"
[ ! -e "$CROSS_ROOTFS_PREFIX/usr/local/include/ffmpeg/avformat.h" -a \
  ! -e "$CROSS_ROOTFS_PREFIX/usr/local/include/libavformat/avformat.h" -a \
  ! -e "$CROSS_ROOTFS_PREFIX/usr/local/include/ffmpeg/libavformat/avformat.h" ] && MISSING="libavformat $MISSING"
[ ! -e "$CROSS_ROOTFS_PREFIX/usr/local/include/ffmpeg/avcodec.h" -a \
  ! -e "$CROSS_ROOTFS_PREFIX/usr/local/include/libavcodec/avcodec.h" -a \
  ! -e "$CROSS_ROOTFS_PREFIX/usr/local/include/ffmpeg/libavcodec/avcodec.h" ] && MISSING="libavcodec $MISSING"
if [ -n "$MISSING" ]; then
...
case $OS_NAME in
	...
	Linux)
		...
		;;
  AXELinux)
    OS_URL=http://www.inverto.tv/
    echo "#define PLATFORM_AXE  1" >> ${CONFIGFILE}
    ;;
	*)
...
if [ -f $CROSS_ROOTFS_PREFIX/usr/include/sys/inotify.h ]; then
...
if [ -f $CROSS_ROOTFS_PREFIX/usr/include/iconv.h ]; then
...
if [ -f $CROSS_ROOTFS_PREFIX/usr/include/libintl.h ]; then
