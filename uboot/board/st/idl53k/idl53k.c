...
{
	STPIO_SET_PIN(PIO_PORT(3), 2, (val) ? 1 : 0);
}

extern void stx7105_spi_sda(const int val)
{
	STPIO_SET_PIN(PIO_PORT(3), 1, (val) ? 1 : 0);
}

extern unsigned char stx7105_spi_read(void)
{
	return STPIO_GET_PIN(PIO_PORT(3), 1);
}

extern void stx7105_spi_cs(int val)
{
	STPIO_SET_PIN(PIO_PORT(3), 3, (val) ? 1 : 0);
}

extern void stx7105_spi_data(int in)
{
	SET_PIO_PIN(PIO_PORT(3), 1 , in ? STPIO_IN : STPIO_OUT);
}
...
extern U32 board_getConfig(int useEnv)
{
	...
	return readl(PIO_PORT(6) + STPIO_PIN_OFFSET) & PIO_CONFIG_MASK;
}
#endif
