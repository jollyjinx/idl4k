...
static void idl52k_hwcontrol(struct mtd_info *mtdinfo, int cmd)
{
 ...
 */
#ifndef CFG_NAND_FLEX_MODE	/* for "bit-banging" (c.f. STM "flex-mode")  */
static int idl52k_device_ready(struct mtd_info *mtd)
{
...
extern int board_nand_init(struct nand_chip * const nand)
{
	...
	stm_default_board_nand_init(nand, idl52k_hwcontrol, idl52k_device_ready);
#endif
